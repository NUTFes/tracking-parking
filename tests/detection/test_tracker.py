...
def make_tracker_and_detector(margin_px=5.0, endpoint_margin_px=0.0,
                               max_gap_sec=90, cleanup_threshold_sec=150):
    ...
    tracker = VehicleTracker(max_gap_sec=max_gap_sec, cleanup_threshold_sec=cleanup_threshold_sec)
    return tracker, detector


def process_frame(tracker, detector, track_id, point, stream_time_sec):
    ...
    tracker.resolve_pending_confidences(stream_time_sec)
...
def test_jitter_across_line1_does_not_double_count():
    ...
    for stream_time_sec, point in enumerate(points):
        process_frame(tracker, detector, track_id=1, point=point, stream_time_sec=stream_time_sec)
...
def test_slow_crossing_across_line1_counts_exactly_once():
    ...
    state = None
    for stream_time_sec, point in enumerate(points):
        state = process_frame(tracker, detector, track_id=1, point=point, stream_time_sec=stream_time_sec)

    assert tracker.total_in == 1
    assert state.counted is True

    # 同じ側に留まったまま再度フレームを処理しても二重カウントしない
    process_frame(tracker, detector, track_id=1, point=(50, 18), stream_time_sec=len(points))
    assert tracker.total_in == 1
...
def test_line2_jitter_does_not_corrupt_passed_order():
    ...
    for stream_time_sec, point in enumerate(line1_crossing_points):
        state = process_frame(tracker, detector, track_id=1, point=point, stream_time_sec=stream_time_sec)
    ...
    for offset, point in enumerate(line2_jitter_points):
        state = process_frame(
            tracker, detector, track_id=1, point=point, stream_time_sec=next_frame_id + offset
        )
...
def test_track_id_reuse_after_cleanup_gets_fresh_transition_state():
    tracker, detector = make_tracker_and_detector(margin_px=5, cleanup_threshold_sec=10)

    process_frame(tracker, detector, track_id=1, point=(50, -12), stream_time_sec=0)
    old_state = tracker.get_state(1)
    assert old_state.line1_transition.stable_side == -1

    tracker.cleanup_stale_tracks(current_time_sec=0 + tracker.cleanup_threshold_sec + 1)
    ...
    new_state = process_frame(tracker, detector, track_id=1, point=(50, 80), stream_time_sec=100)
...
def test_cleanup_threshold_boundary_exact_and_plus_one():
    tracker, _ = make_tracker_and_detector(cleanup_threshold_sec=10)
    tracker.update(track_id=1, point=(0, 0), stream_time_sec=0)

    tracker.cleanup_stale_tracks(current_time_sec=10)
    assert tracker.get_state(1) is not None

    tracker.cleanup_stale_tracks(current_time_sec=11)
    assert tracker.get_state(1) is None


def test_stream_time_zero_line1_crossing_recorded():
    # ヒステリシス方式では初回観測フレーム自体では交差が確定しない設計のため
    # (side反転の相手が存在しないため)、stream_time_sec=0での交差確定そのものは
    # 起こり得ない。ここでは record_line1_crossing 以降のカウント経路が
    # line1_time の値(0.0はfalsy)に依存していないことを直接確認する。
    tracker, _ = make_tracker_and_detector()
    state = tracker.update(track_id=1, point=(50, 20), stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=0)

    assert state.line1_time == 0
...
def test_large_frame_gap_single_jump_across_line1_still_counts():
    ...
    process_frame(tracker, detector, track_id=1, point=(50, -12), stream_time_sec=0)
    process_frame(tracker, detector, track_id=1, point=(50, 18), stream_time_sec=50)
...
def test_in_line1_then_line2_resolves_high_after_line2():
    ...
    states = []
    for stream_time_sec, point in enumerate(points):
        states.append(process_frame(tracker, detector, 1, point, stream_time_sec))
...
def test_out_line2_then_line1_resolves_high_on_line1_frame():
    ...
    for stream_time_sec, point in enumerate(points):
        state = process_frame(tracker, detector, 1, point, stream_time_sec)
...
def test_out_single_frame_double_crossing_resolves_high_in_physical_order():
    ...
    process_frame(tracker, detector, 1, (50, 61), stream_time_sec=0)
    state = process_frame(tracker, detector, 1, (50, -11), stream_time_sec=1)
...
def test_in_single_frame_double_crossing_resolves_high_in_physical_order():
    ...
    process_frame(tracker, detector, 1, (50, -11), stream_time_sec=0)
    state = process_frame(tracker, detector, 1, (50, 61), stream_time_sec=1)
...
def test_same_frame_order_uses_crossing_points_when_start_points_differ():
    ...
    process_frame(tracker, detector, 1, (50, 61), stream_time_sec=0)
    process_frame(tracker, detector, 1, (50, 58), stream_time_sec=1)
    state = process_frame(tracker, detector, 1, (50, 54), stream_time_sec=2)

    assert state.line1_transition.last_stable_point == (50, 54)
    assert state.line2_transition.last_stable_point == (50, 58)

    state = process_frame(tracker, detector, 1, (50, -11), stream_time_sec=3)
...
def test_pending_in_resolves_normal_after_gap_exceeds_boundary():
    tracker, _ = make_tracker_and_detector(max_gap_sec=3)
    state = tracker.update(1, (50, 10), stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=0)
...
def test_reverse_order_in_is_normal():
    tracker, _ = make_tracker_and_detector()
    state = tracker.update(1, (50, 10), stream_time_sec=0)
    state.record_line2_crossing("IN", stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=1)
    tracker.mark_as_counted(1)
...
def test_direction_mismatch_is_normal():
    ...
    state = tracker.update(1, (50, 10), stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=0)
    state.record_line2_crossing("OUT", stream_time_sec=1)
    tracker.mark_as_counted(1)
...
def test_duplicate_line2_before_line1_is_normal_for_out():
    tracker, _ = make_tracker_and_detector()
    state = tracker.update(1, (50, 10), stream_time_sec=0)
    state.record_line2_crossing("OUT", stream_time_sec=0)
    state.record_line2_crossing("OUT", stream_time_sec=1)
    state.record_line1_crossing("OUT", stream_time_sec=2)
    tracker.mark_as_counted(1)
...
def test_correct_pair_beyond_gap_is_normal():
    ...
    state = tracker.update(1, (50, 10), stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=0)
    state.record_line2_crossing("IN", stream_time_sec=4)
    tracker.mark_as_counted(1)
...
def test_pending_state_survives_cleanup_until_confidence_is_resolved():
    tracker, _ = make_tracker_and_detector(max_gap_sec=90, cleanup_threshold_sec=10)
    state = tracker.update(1, (50, 10), stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=0)
    tracker.mark_as_counted(1)
...
def test_frame_zero_line1_event_starts_pending_confidence():
    tracker, _ = make_tracker_and_detector()
    state = tracker.update(1, (50, 20), stream_time_sec=0)
    state.record_line1_crossing("IN", stream_time_sec=0)
